# WoolCraft — Nonprofit / Charity CRM Template
...
All data lives in **`data.js`** at the project root. Edit this single file to create your own WoolCraft — no store, Redux, or API needed. Each page imports from this data via `app/admin/data.js`, which re-exports from `data.js` only.
...
The app uses a single **Layout** (`app/admin/Layout.jsx`): sidebar on the left (WoolCraft branding, nav from `data.js`, annual goal widget, dark/light toggle), main content on the right with header (breadcrumb, search, notifications, AI button, settings, user avatar). The AI chatbot sidebar is included and unchanged.
